...
def arrange_data(image_files, captions, image_urls, lang):  # iterates through all the captions and save there translations
    try:
        lis_ = []
        if lang == "en":
            ...
            # for image_file, caption, image_url in zip(tqdm(image_files, total=len(image_files), position=0, leave=False, desc=f"processing for {lang} currently"), output, image_urls):  # add other captions
            for image_file, caption, image_url in zip(image_files, output, image_urls):  # add other captions
                lis_.append({"image_file":image_file, "caption":caption, "url":image_url, "lang_id": lang})

            gc.collect()
        return lis_

    except Exception as e:
        print(e, image_file, " skipped!")
        return
...
for i in tqdm(range(0,len(df),BATCH_SIZE)):
    lang = LANG_LIST[roulette % len(LANG_LIST)]
    output_batch = arrange_data(list(df["image_file"])[i:i+BATCH_SIZE], list(df["caption"])[i:i+BATCH_SIZE], list(df["url"])[i:i+BATCH_SIZE], lang)
    roulette += 1
